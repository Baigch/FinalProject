...
function initConsumerEvent() {
    ...
    // Live购买成功通知
    onelive.BuyLive().watch(function(error, result){
        if (!error) {
            console.log("BuyLive Ok");
            showAccount();
            prompt(result.args.consumer + " 购买Live成功，商品编号 " + txMap[result.args.txIndex].goodsId + " 商品数量 " + txMap[result.args.txIndex].quantity);
            saveBuyInfo(result.args.txIndex, result.args.beginUserId)
        }
    });
    // 奖品Live售罄，提交原始随机数种子通知
    onelive.NotifySubmitPlaintext().watch(function(error, result){
    	if (!error) {
            console.log("receive NotifySubmitPlaintext");
            prompt("奖品编号 " + result.args.goodsId + " Live 已售罄，开始提交原始随机数种子");
            submitPlaintext(result.args.goodsId);
        }
    });
    // 原始随机数种子提交成功提示
    onelive.SubmitPlaintext().watch(function(error, result){
        if (!error) {
    ...
    // 开奖通知
    onelive.NotifyWinnerResult().watch(function(error, result){
        if (!error) {
    ...
    // 发送代币成功通知
    onelivecoin.Transfer().watch(function(error, result){
        if (!error) {
...
function showGoodsInfo(goodsId) {
    var goods = onelive.goods.call(goodsId);
...
// 购买Live
function buyLive(addr, goodsId, quantity, plaintext) {
    var ciphertext = sha3(plaintext);
    console.log("buyLive:", addr, goodsId, quantity, plaintext, ciphertext);
    var tx = new Object();
    tx.addr = addr;
    tx.goodsId = goodsId;
    ...
    onelive.buyLive.sendTransaction(goodsId, quantity, ciphertext, txIndex++, {from: addr, gas: 10000000});
}
...
function submitPlaintext(goodsId) {
    ...
    $.each(buyRecordArr, function(index, buyRecord) {
        ...
        onelive.submitPlaintext.sendTransaction(buyRecord.goodsId, buyRecord.beginUserId, buyRecord.plaintext, txIndex++, {from: buyRecord.addr, gas: 10000000});
    });
...
function queryConsumers(goodsId) {
    ...
    var userInfoArr = onelive.user.call(goodsId, userId);
...
function transfer(sender, receiver, value) {
    ...
    onelivecoin.transfer.sendTransaction(receiver, value, {from: sender, gas: 10000000});
}
